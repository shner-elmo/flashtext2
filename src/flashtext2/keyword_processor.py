...
import io
import itertools
from typing import Iterator, Literal, overload, TYPE_CHECKING, TypeAlias

from flashtext2.trie_dict import TrieDict

if TYPE_CHECKING:
    SpanInfo: TypeAlias = tuple[str, int, int]
...
class KeywordProcessor(TrieDict):
    ...
    @overload
    def extract_keywords(self, sentence: str, span_info: Literal[True]) -> list[SpanInfo]:
        ...

    def extract_keywords(
            self, sentence: str, span_info: bool = False) -> list[str] | list[SpanInfo]:
        """
    ...
    def _extract_keywords_iter(self, sentence: str) -> Iterator[str]:
        ...
        # empty string is for running the `while` loop an extra iteration
    ...
    def _extract_keywords_with_span_iter(self, sentence: str) -> Iterator[SpanInfo]:
        """
    ...
    def replace_keywords(self, sentence: str) -> str:
        ...
        # after replacing all the keywords we need to get the text between the last keyword
